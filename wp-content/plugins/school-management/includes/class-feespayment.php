		...
		else
		{
			...
			if(get_option( 'smgt_enable_feesalert_mail')==1)
			{
				...
				foreach($parent as $p)
				{
					...
					$MessageContent	= GetHTMLContent($fees_pay_id);	
					$MessageContent = $data. $MessageContent;	
					wp_mail($eamil,'test mail',$MessageContent,$headers);										
				} 
			}
			else
			{
...
{	
	...
	if(isset($_POST['fees_pay_id']))
		...
		$email_message = "<pre style='font-size:18px; font-family:Times New Roman'>". string_replacement($SearchArr,get_option('payment_recived_mailcontent'))."</pre>";
		$url = site_url(). "/wp-admin/admin.php?page=smgt_fees_payment&print=pdf&payment_id={$fees_pay_id}&fee_paymenthistory=fee_paymenthistory";
		$fileatt = "test.pdf"; // Path to the file 
		$fileatt_name = "invoice.pdf"; // Path to the file 
		$fileatt_type = "application/pdf"; // File Type 						
		$email_from = "YashNand <purchase@yashnandeng.com>"; // Who the email						
		$file = fopen($url,'rb'); 						

...
{
	$user_id = $_REQUEST['add_single_student_mark'];
	...
	foreach($student as $user)
	{
		...
		{			
			...
			else
			{
				...
				//echo " ".$sub_id->subid." ";
				echo '<td><label>Mark</label><BR><input type="text" name="marks_'.$user->ID.'_'.$sub_id->subid.'_mark" value="'.$marks.'" class="form-control validate[required,custom[onlyNumberSp]] text-input"><BR><label>Comment</label><BR><input type="text" name="marks_'.$user->ID.'_'.$sub_id->subid.'_comment" value="'.$marks_comment.'" class="form-control text-input"><input type="hidden" value="'.$mark_id.'" name="marks_'.$user->ID.'_'.$sub_id->subid.'_mark_id"></td>';
			}
		}

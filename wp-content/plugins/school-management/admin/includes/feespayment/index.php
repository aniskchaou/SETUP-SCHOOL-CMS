	...
	{		
		...
		else
		{
			if(!$obj_fees->is_duplicat_fees($_POST['fees_title_id'],$_POST['class_id']))
			{
				...
				if($result)
				{
					wp_redirect ( admin_url() . 'admin.php?page=smgt_fees_payment&tab=feeslist&message=fee_add');
				}
			}
			else
			{
...
{	
    ...
    {	
		...
		{		
			...
			if($result)
			{
				wp_redirect ( admin_url() . 'admin.php?page=smgt_fees_payment&tab=feespaymentlist&message=feetype_add');
			}			
		}
    }	
}

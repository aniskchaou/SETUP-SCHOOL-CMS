...
$(document).ready(function() {
	 $('#grade_form').validationEngine({promptPosition : "bottomRight",maxErrorsPerField: 1});
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<input id="grade_point" class="form-control validate[required,custom[onlyNumberSp],maxSize[5]] text-input" type="number" value="<?php if($edit){ echo $grade_data->grade_point;}?>" name="grade_point">
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<input id="mark_from" class="form-control validate[required,custom[onlyNumberSp]] text-input" type="number" value="<?php if($edit){ echo $grade_data->mark_from;}?>" name="mark_from">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="mark_upto"><?php _e('Mark Upto','school-mgt');?><span class="require-field">*</span></label>
			<div class="col-sm-8">
				<input id="mark_upto" class="form-control validate[required,custom[onlyNumberSp],maxSize[4]] text-input" type="number" value="<?php if($edit){ echo $grade_data->mark_upto;}?>" name="mark_upto">
			</div>
		</div>
		<div class="form-group">

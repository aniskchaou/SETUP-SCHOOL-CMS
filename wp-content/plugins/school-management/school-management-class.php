...
class School_Management
{
	...
	function __construct($user_id = NULL)
	{
		if($user_id)
		{
			...
			{
				$this->role= "teacher";
				if(get_option('smgt_students_access')=='own'){
	...
	{
		$exlude_id = smgt_approve_student_list();
		$list_access = get_option('smgt_student_list_access');
		if($this->role == 'student' && $list_access == 'own_class')
		{
			$students = get_users(array('meta_key' => 'class_name', 'meta_value' => $class_id, 'role'=>'student','include'=>[get_current_user_id()]));
		}
		else{
			$students = get_users(array('meta_key' => 'class_name', 'meta_value' => $class_id, 'role'=>'student','exclude'=>$exlude_id));
		}

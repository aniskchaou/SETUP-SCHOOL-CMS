...
class SubjectListing
{
    ...
    public function subject_listing($data)
    {
        ...
        $array_test    = array();
        /* if($data['teacher_id']!="" && $data['teacher_id']!=0)            $array_test[] = 'teacher_id = '.$data['teacher_id'];        if($data['class_id']!="" && $data['class_id']!=0)            $array_test[] = 'class_id = '.$data['class_id'];        if($data['section_id']!="" && $data['section_id']!=0)            $array_test[] = 'section_id = '.$data['section_id'];  */
        ...
        $sql           = "SELECT * FROM $table_name where class_id = $class_id";
        /* $test_string = implode(" AND ",$array_test);        if(!empty($array_test))        {            $sql .= " Where ";        }        $sql .= $test_string; */

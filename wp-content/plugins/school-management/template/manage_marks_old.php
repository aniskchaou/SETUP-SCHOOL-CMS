		...
		if($result)
					{?>
						<div id="message" class="updated below-h2">
								<p><?php _e('Mark Successfully Updated!','school-mgt');?></p>
							</div>
			  <?php }
...
<div class="panel-body panel-white">  	
 <ul class="nav nav-tabs panel_tabs" role="tablist">
    ...
    <div class="form-group col-md-2">
    	<label for="exam_id"><?php _e('Select Exam','school-mgt');?></label>
        <?php
					$tablename="exam"; 
					$retrieve_class = get_all_data($tablename);?>
            	<select name="exam_id" class="form-control validate[required]" style="line-height: 28px">
	...
	<?php  
       ...
       <select name="class_id"  id="class_list" class="form-control validate[required]" style="line-height: 28px">
    ...
    <?php
		...
		foreach ( $student as $user ) {
			...
			echo '<td>';
			echo '<input type="text" name="marks_'.$user->ID.'" value="'.(isset($csv)&& !(isset($key))? $csv[$key]['marks'] : $marks).'" class="form-control validate[required,custom[onlyNumberSp]] text-input">';
			echo '</td>';
			
			echo '<td>';
			echo '<input type="text" name="comment_'.$user->ID.'" value="'.(isset($csv)&& !(isset($key))? $csv[$key]['comment'] : $marks_comment).'" class="form-control">';
			echo '</td>';
			echo '<td>';

...
if($access)
{
		...
		if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete')
		{
			...
			{?>
				<div id="message" class="updated below-h2">
					<p><?php _e('Book successfully Delete!','school-mgt');?></p>
				</div>
		<?php 
			}
		}
	...
	{		
		...
		if(isset($_REQUEST['action'])&& $_REQUEST['action']=='delete' && $_REQUEST['tab']=='issuelist' && isset($_REQUEST['issuebook_id']))
		{
			...
			{?>
				<div id="message" class="updated below-h2">
					<p><?php _e('Issue Book successfully Delete!','school-mgt');?></p>
				</div>
		<?php 
			}
		}
...
if(isset($_POST['save_book']))
{
	...
	if ( wp_verify_nonce( $nonce, 'save_book_frontend_nonce' ) )
	{
		if($_REQUEST['action']=='edit')
		{
			...
			if($result)
			{
				wp_redirect ( home_url() . '?dashboard=user&page=library&tab=booklist&message=2');	
				?>			
		  <?php }
		}
		else
...
if(isset($_REQUEST['message']))
{
	...
	{ ?>
		<div id="message" class="updated below-h2 "><p>
			<?php _e('Record Successfully Inserted!','school-mgt');	?></p></div>
	<?php 			
	}
	elseif($message == 2){ ?>
		<div id="message" class="updated below-h2 ">
			<p><?php _e("Book Updated Successfully",'school-mgt');?></p>
		</div>
	<?php 			
	}
}
if(isset($_POST['save_issue_book']))
{
	...
	if ( wp_verify_nonce( $nonce, 'issue_book_frontend_nonce' ) )
	{
		if($_REQUEST['action']=='edit')
		{
			...
			{ ?>
				<div id="message" class="updated below-h2">
					<p><?php _e('Issue Updated Successfully','school-mgt');?></p>
				</div>
		  <?php }
		...
		else
		{
				...
				{ ?>
					<div id="message" class="updated below-h2">
						<p><?php _e('Issue Book Added Successfully','school-mgt');?></p>
					</div>
		  <?php }
...
{	
	...
	$('#bookissue_form').validationEngine({promptPosition : "bottomRight",maxErrorsPerField: 1});
...
jQuery(document).ready(function() {
	...
	?>
		...
		<div class="form-group">
			...
			<div class="col-sm-8">
				<select name="period_id" id="category_data" class="form-control issue_period validate[required]">
			...
			<div class="col-sm-2">
				<button id="addremove" class="btn_top" model="period_type"><?php _e('Add Or Remove','school-mgt');?></button>
			</div>
		</div>
